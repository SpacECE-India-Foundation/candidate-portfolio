...
$result = mysqli_query($con,"SELECT DISTINCT quiz.eid,quiz.sahi,quiz.wrong,quiz.time,quiz.title,quiz.date,quiz.total,quiz.intro,quiz.tag,quiz.department from quiz JOIN user WHERE quiz.department=user.department AND user.department='$dep_id' ORDER BY quiz.date DESC") or die('Error');
...
{
$eid=@$_GET['eid'];
$q=mysqli_query($con,"SELECT * FROM `history`WHERE eid='$eid' AND email='$uname' " )or die('Error157');
